...
DbFetcher.prototype.processData = function (data) {
	...
	data.forEach((row) => {
		if (!this.config.ignoredStations.includes(row.station.id) && !this.config.ignoredDirections.includes(row.direction)) {
			console.log('------------------------------------------------------------------------------');
			console.log(row);
			console.log('------------------------------------------------------------------------------');
...
function printDeparture(row) {
	...
	let time = row.when.toLocaleTimeString([], {
		hour: '2-digit',
		minute: '2-digit'
	});
